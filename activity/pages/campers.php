...
if (isset($_GET['sort'])) {
	switch ($_GET['sort']) {
		case "lna":
			...
			break;
		case "awddte":
			$order = "order by CONVERT(INT,SUBSTRING([Month/Year],3,4)) desc,
				CONVERT(INT,SUBSTRING([Month/Year],1,CHARINDEX('/',[Month/Year]) - 1)) desc";
			break;
		default:
			$order = "order by LastName asc,FirstName asc";
	}
...
if (isset($_GET['year']) && strlen($_GET['year']) == 4) {
	$where = "WHERE (`Month/Year` LIKE CONCAT('%'," . $_GET['year'] . ")
		OR YEAR(MaxDateAwarded) = " . $_GET['year'] . ")";
}
